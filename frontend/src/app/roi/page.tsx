'use client';

import { useState, useEffect, useRef } from 'react';
import { motion } from 'framer-motion';
import { api } from '@/lib/api';
import PageHeader from '@/components/layout/PageHeader';
import type { FleetROI, BeforeAfterComparison, WhatIfScenario, WhatIfResult } from '@/types/fleet';
import {
  DollarSign, TrendingUp, TrendingDown, Shield, Fuel, Users, Zap, Calculator,
  Loader2, ArrowRight, ChevronDown,
} from 'lucide-react';

function AnimatedNumber({ value, prefix = '', suffix = '' }: { value: number; prefix?: string; suffix?: string }) {
  const [display, setDisplay] = useState(0);
  const ref = useRef<number>(0);

  useEffect(() => {
    const duration = 1500;
    const start = ref.current;
    const startTime = Date.now();
    const tick = () => {
      const elapsed = Date.now() - startTime;
      const progress = Math.min(1, elapsed / duration);
      const eased = 1 - Math.pow(1 - progress, 3);
      const current = Math.round(start + (value - start) * eased);
      setDisplay(current);
      if (progress < 1) requestAnimationFrame(tick);
      else ref.current = value;
    };
    requestAnimationFrame(tick);
  }, [value]);

  return <>{prefix}{display.toLocaleString()}{suffix}</>;
}

const savingsIcons: Record<string, React.ReactNode> = {
  insurance: <Shield className="w-5 h-5 text-blue-500" />,
  accident: <Zap className="w-5 h-5 text-red-500" />,
  fuel: <Fuel className="w-5 h-5 text-amber-500" />,
  retention: <Users className="w-5 h-5 text-purple-500" />,
  productivity: <TrendingUp className="w-5 h-5 text-emerald-500" />,
};

export default function ROIPage() {
  const [roi, setROI] = useState<FleetROI | null>(null);
  const [beforeAfter, setBeforeAfter] = useState<BeforeAfterComparison | null>(null);
  const [scenarios, setScenarios] = useState<WhatIfScenario[]>([]);
  const [whatIfResults, setWhatIfResults] = useState<WhatIfResult[]>([]);
  const [selectedScenario, setSelectedScenario] = useState<string>('');
  const [retention, setRetention] = useState<{ driversAtRisk: number; totalRetentionCostAtRisk: number; projectedSavings: number; details: { driverId: string; driverName: string; burnoutRisk: string; retentionCost: number }[] } | null>(null);
  const [loading, setLoading] = useState(true);

  const load = async () => {
    setLoading(true);
    try {
      const [r, ba, sc, ret] = await Promise.all([
        api.fleetROI(), api.beforeAfter(), api.whatIfDefaults(), api.retentionSavings(),
      ]);
      setROI(r); setBeforeAfter(ba); setScenarios(sc); setRetention(ret);
      // Run all scenarios
      if (sc.length > 0) {
        const results = await api.whatIfSimulate(sc);
        setWhatIfResults(results);
        setSelectedScenario(sc[0].id);
      }
    } catch (err) { console.error(err); }
    setLoading(false);
  };

  useEffect(() => { load(); }, []);

  if (loading || !roi) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <Loader2 className="w-6 h-6 animate-spin text-[#0078D3]" />
      </div>
    );
  }

  const selectedResult = whatIfResults.find((r) => r.scenarioId === selectedScenario);
  const savingsBreakdown = [
    { key: 'insurance', label: 'Insurance Premium', value: roi.insurancePremiumSavings },
    { key: 'accident', label: 'Accident Prevention', value: roi.accidentPreventionSavings },
    { key: 'fuel', label: 'Fuel Savings', value: roi.fuelSavings },
    { key: 'retention', label: 'Driver Retention', value: roi.retentionSavings },
    { key: 'productivity', label: 'Productivity', value: roi.productivityGains },
  ];
  const maxSaving = Math.max(...savingsBreakdown.map((s) => s.value));

  return (
    <>
      <PageHeader title="ROI Dashboard" subtitle="Quantified value of your safety investment" onRefresh={load} />

      <div className="p-6 space-y-5 max-w-[1400px]">
        {/* Hero Banner */}
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
          className="bg-gradient-to-r from-[#0078D3] to-[#1a2332] rounded-xl p-6 text-white">
          <div className="grid grid-cols-4 gap-6 items-center">
            <div className="col-span-1">
              <div className="text-blue-200 text-[0.65rem] uppercase tracking-wider font-medium">Total Annual Savings</div>
              <div className="text-4xl font-extrabold mt-1 text-emerald-300">
                <AnimatedNumber value={roi.totalAnnualSavings} prefix="$" />
              </div>
              <div className="text-blue-200/60 text-[0.7rem] mt-1">vs. investment of ${roi.investmentCost.toLocaleString()}</div>
            </div>
            <div className="text-center">
              <div className="text-blue-200 text-[0.65rem] uppercase tracking-wider font-medium">ROI</div>
              <div className="text-3xl font-bold mt-1">{roi.roiPercent}%</div>
              <div className="text-blue-200/60 text-[0.7rem]">return on investment</div>
            </div>
            <div className="text-center">
              <div className="text-blue-200 text-[0.65rem] uppercase tracking-wider font-medium">Payback Period</div>
              <div className="text-3xl font-bold mt-1">{roi.paybackMonths}<span className="text-lg"> mo</span></div>
              <div className="text-blue-200/60 text-[0.7rem]">to break even</div>
            </div>
            <div className="text-center">
              <div className="text-blue-200 text-[0.65rem] uppercase tracking-wider font-medium">3-Year Value</div>
              <div className="text-3xl font-bold mt-1 text-emerald-300">${Math.round(roi.projectedThreeYearValue / 1000)}K</div>
              <div className="text-blue-200/60 text-[0.7rem]">projected net value</div>
            </div>
          </div>
        </motion.div>

        {/* Savings Breakdown */}
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          <h2 className="text-[0.9rem] font-bold text-gray-900 mb-4 flex items-center gap-2">
            <DollarSign className="w-4 h-4 text-emerald-500" />
            Savings Breakdown
          </h2>
          <div className="space-y-3">
            {savingsBreakdown.map((item, i) => (
              <motion.div key={item.key} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                className="flex items-center gap-3">
                <div className="w-8">{savingsIcons[item.key]}</div>
                <div className="w-36 text-[0.8rem] font-medium text-gray-700">{item.label}</div>
                <div className="flex-1">
                  <div className="w-full bg-gray-100 rounded-full h-6">
                    <motion.div
                      initial={{ width: 0 }}
                      animate={{ width: `${maxSaving > 0 ? (item.value / maxSaving) * 100 : 0}%` }}
                      transition={{ duration: 0.8, delay: i * 0.1 }}
                      className="h-6 rounded-full bg-gradient-to-r from-[#0078D3] to-emerald-400 flex items-center justify-end pr-2"
                    >
                      <span className="text-[0.65rem] font-bold text-white">${item.value.toLocaleString()}</span>
                    </motion.div>
                  </div>
                </div>
              </motion.div>
            ))}
          </div>
        </div>

        <div className="grid grid-cols-12 gap-5">
          {/* Before / After */}
          {beforeAfter && (
            <div className="col-span-7 bg-white rounded-xl border border-gray-200 p-5">
              <h2 className="text-[0.9rem] font-bold text-gray-900 mb-3 flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-[#0078D3]" />
                Before / After Comparison
              </h2>
              <div className="text-[0.65rem] text-gray-400 mb-3">
                {beforeAfter.periods[0].label} vs {beforeAfter.periods[1].label}
              </div>
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left text-[0.7rem] font-semibold text-gray-500 pb-2">Metric</th>
                    <th className="text-right text-[0.7rem] font-semibold text-gray-500 pb-2">Before</th>
                    <th className="text-right text-[0.7rem] font-semibold text-gray-500 pb-2">After</th>
                    <th className="text-right text-[0.7rem] font-semibold text-gray-500 pb-2">Change</th>
                    <th className="text-right text-[0.7rem] font-semibold text-gray-500 pb-2">$ Impact</th>
                  </tr>
                </thead>
                <tbody>
                  {beforeAfter.metrics.map((m) => {
                    const improved = m.name === 'Avg Safety Score' ? m.change > 0 : m.change < 0;
                    return (
                      <tr key={m.name} className="border-b border-gray-50">
                        <td className="py-2 text-[0.78rem] text-gray-700 font-medium">{m.name}</td>
                        <td className="py-2 text-right text-[0.78rem] text-gray-500">{m.before.toLocaleString()}</td>
                        <td className="py-2 text-right text-[0.78rem] text-gray-900 font-medium">{m.after.toLocaleString()}</td>
                        <td className={`py-2 text-right text-[0.78rem] font-semibold flex items-center justify-end gap-1 ${improved ? 'text-emerald-600' : 'text-red-500'}`}>
                          {improved ? <TrendingDown className="w-3 h-3" /> : <TrendingUp className="w-3 h-3" />}
                          {m.changePercent > 0 ? '+' : ''}{m.changePercent}%
                        </td>
                        <td className="py-2 text-right text-[0.78rem] font-semibold text-emerald-600">
                          ${m.dollarImpact.toLocaleString()}
                        </td>
                      </tr>
                    );
                  })}
                </tbody>
              </table>
            </div>
          )}

          {/* What-If Simulator */}
          <div className="col-span-5 bg-white rounded-xl border border-gray-200 p-5">
            <h2 className="text-[0.9rem] font-bold text-gray-900 mb-3 flex items-center gap-2">
              <Calculator className="w-4 h-4 text-[#0078D3]" />
              What-If Simulator
            </h2>
            <div className="relative mb-4">
              <select
                value={selectedScenario}
                onChange={(e) => setSelectedScenario(e.target.value)}
                className="w-full bg-gray-50 border border-gray-200 rounded-lg px-3 py-2.5 text-[0.8rem] text-gray-800 appearance-none pr-8 focus:outline-none focus:border-[#0078D3]"
              >
                {scenarios.map((s) => (
                  <option key={s.id} value={s.id}>{s.name}</option>
                ))}
              </select>
              <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" />
            </div>

            {selectedResult && (
              <motion.div key={selectedResult.scenarioId} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                <div className="flex items-center justify-center gap-3 py-3">
                  <div className="text-center">
                    <div className="text-[0.6rem] text-gray-400 uppercase">Current</div>
                    <div className="text-2xl font-bold text-gray-400">{selectedResult.currentScore}</div>
                    <div className="text-[0.7rem] text-gray-400">{selectedResult.currentGrade}</div>
                  </div>
                  <ArrowRight className="w-5 h-5 text-[#0078D3]" />
                  <div className="text-center">
                    <div className="text-[0.6rem] text-emerald-600 uppercase font-medium">Projected</div>
                    <div className="text-2xl font-bold text-emerald-600">{selectedResult.projectedScore}</div>
                    <div className="text-[0.7rem] text-emerald-600">{selectedResult.projectedGrade}</div>
                  </div>
                </div>

                <div className="bg-emerald-50 rounded-lg p-3 text-center">
                  <div className="text-[0.6rem] text-emerald-600 uppercase font-medium">Annual Premium Savings</div>
                  <div className="text-2xl font-extrabold text-emerald-600">${selectedResult.annualSavings.toLocaleString()}</div>
                </div>

                <div className="grid grid-cols-2 gap-2 text-[0.7rem]">
                  <div className="bg-gray-50 rounded p-2">
                    <span className="text-gray-500">Difficulty: </span>
                    <span className={`font-semibold ${
                      selectedResult.implementationDifficulty === 'easy' ? 'text-emerald-600' :
                      selectedResult.implementationDifficulty === 'moderate' ? 'text-amber-600' : 'text-red-600'
                    }`}>{selectedResult.implementationDifficulty}</span>
                  </div>
                  <div className="bg-gray-50 rounded p-2">
                    <span className="text-gray-500">Timeline: </span>
                    <span className="font-semibold text-gray-700">{selectedResult.timeToImpact}</span>
                  </div>
                </div>

                <div className="space-y-1.5">
                  {selectedResult.recommendations.map((r, i) => (
                    <div key={i} className="text-[0.7rem] text-gray-600 flex gap-1.5">
                      <span className="text-[#0078D3]">&#8226;</span> {r}
                    </div>
                  ))}
                </div>
              </motion.div>
            )}
          </div>
        </div>

        {/* Retention Risk Table */}
        {retention && retention.details.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-[0.9rem] font-bold text-gray-900 flex items-center gap-2">
                <Users className="w-4 h-4 text-purple-500" />
                Retention Risk
              </h2>
              <div className="bg-purple-50 rounded-lg px-3 py-1.5 text-[0.75rem]">
                <span className="text-purple-600 font-bold">
                  Intervening now could save ${retention.projectedSavings.toLocaleString()}
                </span>
              </div>
            </div>
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-[0.7rem] font-semibold text-gray-500 pb-2">Driver</th>
                  <th className="text-left text-[0.7rem] font-semibold text-gray-500 pb-2">Burnout Risk</th>
                  <th className="text-right text-[0.7rem] font-semibold text-gray-500 pb-2">Retention Cost at Risk</th>
                </tr>
              </thead>
              <tbody>
                {retention.details.map((d) => (
                  <tr key={d.driverId} className="border-b border-gray-50">
                    <td className="py-2 text-[0.8rem] font-medium text-gray-900">{d.driverName}</td>
                    <td className="py-2">
                      <span className={`px-2 py-0.5 rounded-full text-[0.65rem] font-semibold ${
                        d.burnoutRisk === 'high' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                      }`}>{d.burnoutRisk}</span>
                    </td>
                    <td className="py-2 text-right text-[0.8rem] font-bold text-red-600">
                      ${d.retentionCost.toLocaleString()}
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>
    </>
  );
}
