...
async function fetchAndPopulate(): Promise<void> {
  ...
  // Step 1: Fetch devices, users, rules, exception events in parallel
  const [devices, users, rules, exceptionEvents] = await Promise.all([
  ...
  if (hasRealDrivers) {
    ...
    // Synthesize driver users from devices (1 driver per device)
    effectiveDrivers = activeDevices.map((d, i) => {
      ...
      return {
        id: syntheticId,
        name: `Driver ${d.name.replace(/^Demo\s*-\s*/, '').trim()}`,
        firstName: `Driver`,
        lastName: d.name.replace(/^Demo\s*-\s*/, '').trim() || `${i + 1}`,
        isDriver: true,
      } as GeotabUser;
    });
  ...
  const mappedVehicles = mapVehicles(activeDevices, deviceIdMap);
...
function mapVehicles(devices: GeotabDevice[], idMap: Map<string, string>): SeedVehicle[] {
  return devices.map((d) => {
    const seedId = idMap.get(d.id) || d.id;
    const odometerKm = (d.odometer || 0) / 1000; // Geotab reports in meters
...
function mapDrivers(
  ...
  return users.map((u) => {
    ...
    // Estimate tenure: we don't have hire date from Geotab, default to 3 years
    const tenureYears = 3;
...
function mapSafetyEvents(
  ...
  return events.map((e, i) => {
    ...
    return {
      ...
      latitude: 33.7 + Math.random() * 5, // Geotab exception events don't include lat/lng directly
      longitude: -84.4 + Math.random() * 10,
      details: `${ruleName || eventType.replace(/_/g, ' ')} event detected`,
    };
...
function mapTripDays(
  ...
  for (const [key, dayTrips] of grouped) {
    ...
    const totalDistanceKm = dayTrips.reduce((s, t) => s + (t.distance || 0), 0) / 1000;
...
function mapFleetKPIs(tripDays: SeedTripDay[], events: SeedSafetyEvent[]): SeedFleetKPI[] {
  ...
  for (const date of dateSet) {
    ...
    // Safety score: 100 - (events per trip * 10), clamped 0-100
    const eventsPerTrip = totalTrips > 0 ? totalEvents / totalTrips : 0;
    const avgSafetyScore = Math.max(0, Math.min(100, 100 - eventsPerTrip * 10));
...
function buildDriverTripStats(trips: GeotabTrip[], drivers: GeotabUser[]): Map<string, DriverTripStats> {
  ...
  for (const driver of drivers) {
    ...
    for (const date of dates) {
      ...
      const dayDistance = dayTrips.reduce((s, t) => s + (t.distance || 0), 0) / 1000;
...
function mapRuleToEventType(ruleName: string): SafetyEventType {
  ...
  return 'speeding'; // Default fallback
}
...
function inferSeverity(event: GeotabExceptionEvent): SeedSafetyEvent['severity'] {
  ...
  if (durationMs > 300000) return 'critical'; // > 5 min
  if (durationMs > 60000) return 'high';       // > 1 min
  if (durationMs > 10000) return 'medium';      // > 10 sec
  return 'low';
}
